...
public interface MessageRepository extends CrudRepository<Message, Integer> {
	...
	@Query(value = "SELECT DISTINCT u FROM User u WHERE u NOT IN (SELECT u1 FROM User u1 WHERE u1 = ?1) AND (u IN (SELECT m.recipient FROM Message m WHERE m.sender = ?1) OR u IN (SELECT DISTINCT m1.sender FROM Message m1 WHERE m1.recipient = ?1))")
